...
import org.apache.cassandra.gui.gui.component.dialog.KeyDialog;
import org.apache.cassandra.gui.gui.component.dialog.KeyRangeDialog;
...
public class KeyspaceTreePanel extends JPanel implements TreeSelectionListener {
    ...
    private class MousePopup extends MouseAdapter {
    	...
    	public void mouseClicked(MouseEvent e) {
			...
			if (node != null && node.getChildCount() == 0) {
				...
				cCallback.rangeCallback(lastSelectedKeysapce,
						lastSelectedColumnFamily, startKey, endKey, row);
    ...
    public KeyspaceTreePanel(Client client) {
        try {
            ...
            List<KsDef> ksDefs = client.getKeyspaces();
...
//            for (String keyspace : ks) {
                DefaultMutableTreeNode keyspaceNode = new DefaultMutableTreeNode(ksDef.getName());
                clusterNode.add(keyspaceNode);
                try {
                	List<CfDef> cfDefs = client.getColumnFamilys(ksDef.getName());
                	for (CfDef cfDef : cfDefs) {
					
//                    Set<String> cfs = client.getColumnFamilys(keyspace);
//                    for (String columnFamily : cfs) {
                        keyspaceNode.add(new DefaultMutableTreeNode(cfDef.getName()));
                        keyspaceMap.put(cfDef.getName(), ksDef.getName());
                    }
