package org.apache.cassandra.gui.gui.component.panel;
...
import org.apache.cassandra.gui.gui.component.dialog.SearchDialog;
import org.apache.cassandra.gui.gui.component.dialog.action.ColumnPopupAction;
import org.apache.cassandra.gui.gui.control.callback.RepaintCallback;
import org.apache.cassandra.gui.node.TreeNode;
...
import org.apache.cassandra.gui.unit.Unit;
...
public class ColumnTreePanel extends JPanel {
    ...
    private void showSearchDialog() {
    	if(searchDialog==null) {
    		searchDialog = new SearchDialog(tree);
    	} else {
    		searchDialog.setJtree(tree);
    	}
...
//            scrollPane.getParent().getHeight();
//            scrollPane.setPreferredSize(preferredSize)
            scrollPane.setPreferredSize(new Dimension(scrollPane.getParent().getWidth() - 5,
            		scrollPane.getParent().getHeight() - 5));
            scrollPane.repaint();
        }
        super.repaint();
    }

    public void showRow(String keyspace, String columnFamily, byte[] key) {
        try {
            CfDef cfDef = client.getColumnFamily(keyspace, columnFamily);
            if (cfDef.getColumn_type().equals(COLUMN_FAMILY_TYPE_SUPER)) {
            ...
            Map<String, Key> l =
                client.getKey(columnFamily, null, key);
            showTree(l);
    ...
    public void showRows(String keyspace, String columnFamily, String startKey, String endKey, int rows) {
        try {
            ...
            Map<String, Key> columnFamilyData = client.listKeyAndValues(keyspace, columnFamily, startKey, endKey, rows);
            showTree(columnFamilyData);
            setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
        } catch (Exception e) {
            setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
            JOptionPane.showMessageDialog(null, "error: " + e.getMessage());
    ...
    private void showTree(Map<String, Key> recordMap) {
        DefaultMutableTreeNode columnFamilyNode = new DefaultMutableTreeNode(client.getColumnFamily());
        treeModel = new DefaultTreeModel(columnFamilyNode);
        tree = new JTree(treeModel);
        tree.setRootVisible(true);
        ...
        for (String keyName : recordMap.keySet()) {
            Key k = recordMap.get(keyName);
            DefaultMutableTreeNode keyNode = new DefaultMutableTreeNode(k.getNameStr());
            k.setTreeNode(keyNode);
            columnFamilyNode.add(keyNode);
            unitMap.put(keyNode, k);
            keyMap.put(k.getNameStr(), k);
            if (k.isSuperColumn()) {
        ...
        Action showSearchDialogAction = new AbstractAction() {			
			@Override
		...
		tree.getInputMap().put(keyStroke, "showSearchDialog");
		
		tree.getActionMap().put("showSearchDialog", showSearchDialogAction);
