package org.apache.cassandra.gui.gui.component.dialog.action;
...
import org.apache.cassandra.gui.gui.component.dialog.CellPropertiesDialog;
...
//        String keyStr = cpdlg.getKey();
        ...
        try {
            ...
            if (k == null) {
                Map<String, Key> m = client.getKey(client.getColumnFamily(),
                                                   superColumn.isEmpty() ?
                                                       null : superColumn,
                                                   key.getBytes());
                k = m.get(key);
                if (k == null) {
                    k = new Key(key.getBytes(), new HashMap<String, SColumn>(), new HashMap<String, Cell>());
        ...
        Date d = null;
        try {
            d = client.insertColumn(client.getColumnFamily(),
                                    key.getBytes(),
    ...
    private String getName() {
        if (treeNode.getUnit() instanceof Key) {
            return ((Key) treeNode.getUnit()).getNameStr();
        } else if (treeNode.getUnit() instanceof SColumn) {
